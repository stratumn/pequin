...
int compute(struct In *input, struct Out *output) {
  ...
  uint32_t exo0_inputs1[1] = {1};
  uint32_t *exo0_inputs_array[1] = {exo0_inputs1};
  uint32_t exo0_lens[1] = {1};

  exo_compute(exo0_inputs_array, exo0_lens, s, 0);

  // hash it
  SHA256_CTX ctx;

  sha256_init(&ctx);
  sha256_update(&ctx, s, 64);
  sha256_final(&ctx, output->hash);

  // compare
  return are_equal(input->hash, output->hash);
}
